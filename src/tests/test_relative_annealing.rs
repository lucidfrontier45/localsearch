...
use crate::optim::{LocalSearchOptimizer, RelativeAnnealing, RelativeAnnealingOptimizer};
...
/// previously produced `inf`/`NaN` from the relative-eps denominator.
#[test]
fn test_initial_score_zero() {
    let centers = vec![2.0, 0.0, -3.5];
    let model = QuadraticModel::new(3, centers.clone(), (-10.0, 10.0));
    let opt = RelativeAnnealingOptimizer::new(500, 10, 100, 1e1);
