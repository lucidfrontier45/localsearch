...
- Tuning helpers:
  - `tune_temperature(model, initial_solution_and_score, n_warmup, target_prob, seed: Option<u64>) -> f64` — tunes inverse temperature from warmup trials (target acceptance probability for uphill moves). `seed = None` preserves the entropy-driven behavior; `Some(s)` makes the warmup stream reproducible.
  - `gather_energy_diffs(model, initial_solution_and_score, n_warmup, seed: Option<u64>) -> Vec<f64>` and `calculate_temperature_from_acceptance_prob(&energy_diffs, target_prob) -> f64` — the lower-level warmup primitives `tune_temperature` is built on; same `seed` semantics.
